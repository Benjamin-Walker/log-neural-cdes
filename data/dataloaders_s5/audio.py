...
from .base import default_data_path, deprecated, SequenceDataset
...
def minmax_scale(tensor, range_min=0, range_max=1):
    ...
    return range_min + (range_max - range_min) * (tensor - min_val) / (
...
class AbstractAudioDataset(torch.utils.data.Dataset):
    ...
    def setup(self):
        return NotImplementedError(
            "Must assign a list of filepaths to self.file_names."
        )
    ...
    def __getitem__(self, index):
        ...
        if self.sample_len is not None:
            ...
            seq, sr = torchaudio.load(
                file_name, frame_offset=start_frame, num_frames=num_frames
            )
        else:
        ...
        if self.target_sr is not None and sr != self.target_sr:
            if sr not in self.transforms:
                self.transforms[sr] = torchaudio.transforms.Resample(
        ...
        if self.return_type == "autoregressive":
            ...
            if self.pad_len is not None:
                x = torch.cat(
                    (torch.zeros(self.pad_len, dtype=self.qtype) + self.zero, x)
                )  # Pad the signal
            return x, y
    ...
    def create_examples(self, sample_len: int):
        ...
        self.metadata = [torchaudio.info(file_name) for file_name in self.file_names]
        ...
        if sample_len is not None:
            ...
            for file_name, metadata in zip(self.file_names, self.metadata):
                ...
                if (
                    ...
                    and metadata.sample_rate != self.target_sr
                ):
                    sample_len_i = int(
                        sample_len * metadata.sample_rate / self.target_sr
                    )
                ...
                if margin > 0 and not self.drop_last:
                    ...
                    self.examples.append(
                        (file_name, metadata.num_frames - margin, margin)
                    )
        else:
...
class QuantizedAudioDataset(AbstractAudioDataset):
    ...
    def setup(self):
        ...
        self.file_names = file_names[
            int(self.ratio_min * len(file_names)) : int(
                self.ratio_max * len(file_names)
            )
        ]
...
class QuantizedAutoregressiveAudio(SequenceDataset):
    ...
    def setup(self):
        ...
        assert (
            self.path is not None or self.data_dir is not None
        ), "Pass a path to a folder of audio: either `data_dir` for full directory or `path` for relative path."
        ...
        def collate_fn(batch):
            ...
            if self.pad_len is None:
                pad_length = int(
                    min(2 ** max_length.log2().ceil(), self.sample_len) - max_length
                )
            else:
                pad_length = int(
                    min(2 ** max_length.log2().ceil(), self.sample_len + self.pad_len)
                    - max_length
                )
...
class SpeechCommands09Autoregressive(SequenceDataset):
    ...
    def _collate_fn(self, batch):
        ...
        x = nn.utils.rnn.pad_sequence(
            x,
            padding_value=self.dataset_train.zero if not self.dequantize else 0.0,
            batch_first=True,
        )
        x = F.pad(
            x,
            (0, pad_length),
            value=self.dataset_train.zero if not self.dequantize else 0.0,
        )
        y = nn.utils.rnn.pad_sequence(
            y,
            padding_value=-100,  # pad with -100 to ignore these locations in cross-entropy loss
...
class MaestroAutoregressive(SequenceDataset):
    ...
    def setup(self):
        ...
        self.data_dir = (
            self.data_dir or default_data_path / self._name_ / "maestro-v3.0.0"
        )
    ...
    def _collate_fn(self, batch):
        ...
        max_length = lengths.max()
        pad_length = int(
            min(max(1024, 2 ** max_length.log2().ceil()), self.sample_len) - max_length
        )
        x = nn.utils.rnn.pad_sequence(
            x,
            padding_value=self.dataset_train.zero,
...
class LJSpeech(QuantizedAudioDataset):
    ...
    def setup(self):
        ...
        if self.use_text:
            ...
            for file_name in self.file_names:
                ...
                self.tok_transcripts[file_name] = list(
                    self.transcripts[file_name].lower()
                )
            ...
            for file_name in self.file_names:
                file_name = file_name.split("/")[-1].split(".")[0]
                self.tok_transcripts[file_name] = torch.tensor(
                    self.label_encoder.transform(self.tok_transcripts[file_name])
                )
    ...
    def __getitem__(self, index):
        ...
        if self.use_text:
            ...
            tok_transcript = self.tok_transcripts[
...
class LJSpeechAutoregressive(SequenceDataset):
    ...
    def setup(self):
        ...
        self.data_dir = (
            self.data_dir or default_data_path / self._name_ / "LJSpeech-1.1" / "wavs"
        )
    ...
    def _collate_fn(self, batch):
        ...
        if self.use_text:
            return (
                x,
                y,
                {"lengths": lengths, "tokens": tokens, "text_lengths": text_lengths},
            )
        else:
            return x, y, {"lengths": lengths}
...
class SpeechCommands09Classification(SequenceDataset):
    ...
    def collate_fn(self, batch):
        ...
        x = F.pad(x, (0, pad_length), value=0.0)  # self.dataset_train.zero)
        y = torch.tensor(y)
        return x, y, {"lengths": lengths}
