...
import torchvision
from dataloaders_s5.utils.util import is_list
...
class DefaultCollateMixin:
    ...
    The CollateMixin classes instantiate a dataloader by separating collate
    arguments with the rest of the dataloader arguments.
    Instantiations of this class should modify the callback functions as
    desired, and modify the collate_args list. The class then defines a
    _dataloader() method which takes in a DataLoader constructor and
    arguments, constructs a collate_fn based on the collate_args, and passes
...
class ImageResolutionCollateMixin(SequenceResolutionCollateMixin):
    """self.collate_fn(resolution, img_size) produces a collate function that
    resizes inputs to size img_size/resolution"""
...
# [21-09-10 AG] Subclassing LightningDataModule fails due to trying to access
# _has_setup_fit. No idea why. So we just provide our own class with the same
# core methods as LightningDataModule (e.g. setup)
class SequenceDataset(DefaultCollateMixin):
    ...
    # TODO it might be possible to write this as a @dataclass, but it seems tricky
    # to separate from the other features of this class
