...
def decode(hilberts, num_dims, num_bits):
    ...
    padded = np.pad(
        gray, ((0, 0), (0, 0), (extra_dims, 0)), mode="constant", constant_values=0
    )
...
def right_shift(binary, k=1, axis=-1):
    ...
    shifted = np.pad(
        binary[tuple(slicing)], padding, mode="constant", constant_values=0
    )
