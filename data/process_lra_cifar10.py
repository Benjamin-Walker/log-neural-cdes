...
def get_cifar10_datasets(n_devices, batch_size=256, normalize=False):
    ...
    def decode(x):
        decoded = {'inputs': tf.cast(tf.image.rgb_to_grayscale(x['image']), dtype=tf.int32),
                   'targets': x['label']}
        if normalize:
...
def get_pathfinder_base_datasets(n_devices, batch_size=256, resolution=32, normalize=False, split='easy'):
    ...
    def decode(x):
        decoded = {'inputs': tf.cast(tf.image.decode_png(x['image']), dtype=tf.int32),
                   'targets': x['label']}
        if normalize:
            decoded['inputs'] = decoded['inputs'] / 255
        return decoded
...
if __name__ == '__main__':
    ...
    if args.task_name.lower() == 'cifar10':
        ...
        (train_dataset, val_dataset, test_dataset, num_classes, vocab_size, input_shape) = get_cifar10_datasets(
            n_devices=jax.local_device_count(),
            batch_size=args.batch_size)
    elif args.task_name.lower() == 'pathfinder':
        _PATHFINER_TFDS_PATH = '/Users/tiexin/Research/DataSets/LRA/TFDS/'
        (train_dataset, val_dataset, test_dataset, num_classes, vocab_size, input_shape) = get_pathfinder_base_datasets(
            n_devices  = jax.local_device_count(),
            batch_size = args.batch_size,
            resolution = 32,
            split = 'hard'
    ...
    n_train, n_val, n_test = len(train_targets_list), len(val_targets_list), len(test_targets_list)
