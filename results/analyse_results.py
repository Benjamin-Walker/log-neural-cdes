    ...
    if os.path.isdir(results_dir + model):
        ...
        for dataset in sorted(os.listdir(results_dir + model)):
                ...
                if len(os.listdir(results_dir + model + dataset + exp)) != 0:
                    ...
                    if model == "S6/" or model == "mamba/":
                        all_train_acc = np.load(
                            results_dir + model + dataset + exp + "/train_acc.npy"
                        )
                    else:
                        all_train_acc = np.load(
                    ...
                    if experiment == "hypopt":
                        if operator(
                            all_train_acc[best_idx(all_val_acc)], best_val(all_val_acc)
                        ):
                            exps.append(exp)
                            val_accs.append(best_val(all_val_acc))
                            train_accs.append(all_train_acc[best_idx(all_val_acc)])
            ...
            if experiment == "hypopt":
                ...
                train_idxs = np.where(train_accs[idxs] == best_val(train_accs[idxs]))[0]
